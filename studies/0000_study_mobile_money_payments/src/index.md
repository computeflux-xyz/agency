...
<div class="hero">
  <h1>Payment is<br>the product</h1>
...
We read the code. We traced the transactions. We verified every claim against the actual implementation in `shared/libs/go/payments/mpesa_drc/` and `bijougabriel-api/application/usecases/payment/mpesa/`. No marketing slides. No hand-waving. Just engineering.
...
const retryRate = view(Inputs.range([0, 0.6], {value: 0.18, step: 0.01, label: "Buyers who re-submit", format: (x) => fmtPct(x, 0)}));
const extraTaps = view(Inputs.range([1, 4], {value: 1.6, step: 0.1, label: "Extra submissions each"}));
const doubleDebitOdds = view(Inputs.range([0, 1], {value: 0.35, step: 0.01, label: "Unguarded duplicate that actually debits", format: (x) => fmtPct(x, 0)}));
...
- **A buyer with no card can complete a purchase** — which is the entire addressable market, not a slice of it.

- **A buyer who taps twice is charged once** — by construction, not by vigilance.

- **A callback that never arrives becomes a delay, not a loss** — because the ledger is reconciled against the rail, not assumed from it.
...
Behind the scenes, the `bijougabriel-admin` backoffice gives the business complete visibility and control over payments:
